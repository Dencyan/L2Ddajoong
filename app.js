...
function prepareMotionText(element, text = element.textContent) {
  ...
  accessibleText.className = "type-accessible";
  ...
  label.split(/\s+/u).filter(Boolean).forEach((wordText) => {
    ...
    fragment.append(word, document.createTextNode(" "));
  });
...
function playMotionText(element) {
  const restarting = element.classList.contains("is-type-visible");
  element.classList.remove("is-type-visible");
  if (restarting) {
    requestAnimationFrame(() => requestAnimationFrame(() => element.classList.add("is-type-visible")));
  } else {
